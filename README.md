...
| **LOOT 记录** | 轻量战利品记录：快速添加、表格内编辑、归属分配、金币均分、导出 CSV；使用 PHP+SQLite 持久化存储 | 首页 → 查看LOOT |
...
6. **LOOT 记录**  
   ...
   - **数据存储**：使用 PHP+SQLite 持久化存储，数据保存在服务器的 `demo/loot.db` 文件中，支持多用户访问。  
   - **服务器要求**：需部署在支持 PHP 的服务器上，并确保启用了 SQLite 扩展（pdo_sqlite、sqlite3）。
...
├── demo/                # LOOT 记录功能
│   ├── log.html         # LOOT 记录页面
│   ├── api.php          # API 接口脚本（PHP+SQLite）
│   ├── init_db.php      # 数据库初始化脚本
│   └── loot.db          # SQLite 数据库文件（自动生成）
├── 小工具/
│   ├── 网站资源集.html
│   ├── 骰子模拟器.html
│   ├── token制作器.html
...
- **LOOT 记录持久化**：依赖 `demo/api.php` 和 `demo/init_db.php`，需 PHP 环境且启用 SQLite 扩展（pdo_sqlite、sqlite3）；使用 SQLite 数据库存储数据。
- **建议环境**：现代浏览器（Chrome、Edge、Firefox、Safari 等）；部署时需支持 PHP 的服务器，并确保 SQLite 扩展已启用。
